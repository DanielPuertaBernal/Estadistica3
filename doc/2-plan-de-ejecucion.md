...
|---|---|---|---|
| T1 | ~~La carga lee un CSV suelto del directorio actual~~ | Protocolo 1.9 | ✅ **Resuelto.** Lee `data/survey_results_public.csv` y escribe en `salidas/stackoverflow_limpio.csv`; todas las rutas se resuelven desde la raíz del repositorio, no desde el directorio de ejecución. Verificado: corre de punta a punta, 64.461 → 63.803 filas. **Ojo:** las rutas ya no son las del protocolo — ver E5 |
...
| E2 | Archivos pesados en el control de versiones | README | ⚠️ **Resuelto a medias, con riesgo vivo.** Se borró la carpeta del compañero (CSV duplicado verificado por hash) y la carpeta `archive/`. Por indicación del profesor, `data/` guarda ahora el CSV original **sin comprimir**: 94.603.888 bytes, 90 MiB. Pasa el límite duro de GitHub (100 MiB) pero dispara su aviso de 50 MB. Como el original nunca cambia, git guarda una sola copia. Todo lo que produce el script se fue a `salidas/`, que está en `.gitignore`. **Riesgo:** si alguien commitea una versión modificada de ese CSV, git guarda otros 90 MiB para siempre |
...
| E5 | **Las rutas de entrada y salida ya no son las del protocolo** | Protocolo 1.9 | El profesor pidió que `data/` contenga únicamente el CSV original sin comprimir, sin el dataset limpio ni el Excel de entrega. Eso obliga a dos desviaciones de la sección 1.9, que fija entrada `data/archive.zip` y salida `data/stackoverflow_limpio.csv`. Hoy el script lee `data/survey_results_public.csv` y escribe `salidas/stackoverflow_limpio.csv`. **No es una decisión del equipo: viene de afuera.** Pero el protocolo está congelado, así que va al reporte de desviaciones con esta causa. Decidir además si el dataset limpio entra al repositorio de alguna forma o solo viaja en el comprimido de entrega |
