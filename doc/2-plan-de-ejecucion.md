...
|---|---|---|---|
| T1 | ~~La carga lee un CSV suelto del directorio actual~~ | Protocolo 1.9 | ✅ **Resuelto.** Lee `data/archive.zip` con `zipfile` sin descomprimir en disco — la entrada que fija el protocolo — y escribe en `salidas/stackoverflow_limpio.csv`. Todas las rutas se resuelven desde la raíz del repositorio, no desde el directorio de ejecución. Verificado: corre de punta a punta, 64.461 → 63.803 filas |
...
| E2 | ~~Archivos pesados en el control de versiones~~ | README | ✅ **Resuelto.** El profesor confirmó que la entrega es un comprimido aparte, no el repositorio, y que el original va comprimido. `data/` guarda solo `archive.zip` (9 MB) y todo lo que produce el script vive en `salidas/`, ignorado. `data/*.csv` también está ignorado, para que un CSV descomprimido no llegue a un commit por accidente. **Nota:** el CSV de 90 MiB y el Excel de 20 MB quedaron en el histórico de git (commits ef84876 y anteriores). Se pueden purgar con `git filter-repo`, pero eso reescribe historia ya publicada y obliga a los tres a re-clonar; no vale la pena salvo que el repositorio se vuelva inmanejable |
...
| E5 | **La ruta de salida no es la del protocolo** | Protocolo 1.9 | La sección 1.9 fija salida `data/stackoverflow_limpio.csv`; el script escribe `salidas/stackoverflow_limpio.csv`. El profesor indicó que el dataset limpio va en `salidas/` y que `data/` guarda únicamente el original. **No es una decisión del equipo: viene de afuera.** El protocolo está congelado, así que va al reporte de desviaciones con esta causa. La entrada (`data/archive.zip`) sí cumple el protocolo. **Redactar el párrafo de esta desviación para el informe** |
